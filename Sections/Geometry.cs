  ...
  internal class Geometry
  {
    ...
    public Geometry(BinaryReader instream, SectionHeader section)
    {
      ...
      foreach (GeometryHeader header in this.headers)
      {
        ...
        else if (header.item_type == 17U)
        {
          ...
          this.unknown_item_data.Add(instream.ReadBytes((int) numArray[header.item_size] * (int) this.vert_count));
      }
    ...
    public void PrintDetailedOutput(StreamWriter outstream)
    {
      ...
      for (int index = 0; index < this.weight_groups.Count; ++index)
        outstream.WriteLine(this.weight_groups[index].Bones1.ToString() + "\t" + this.weight_groups[index].Bones2.ToString() + "\t" + this.weight_groups[index].Bones3.ToString() + "\t" + this.weight_groups[index].Bones4.ToString() + "\t" + this.weights[index].X.ToString("0.000000", (IFormatProvider) CultureInfo.InvariantCulture) + "\t" + this.weights[index].Y.ToString("0.000000", (IFormatProvider) CultureInfo.InvariantCulture) + "\t" + this.weights[index].Z.ToString("0.000000", (IFormatProvider) CultureInfo.InvariantCulture) + "\t" + (this.weights[index].X + this.weights[index].Y + this.weights[index].Z).ToString("0.000000", (IFormatProvider) CultureInfo.InvariantCulture) + "\t" + this.unknown20[index].X.ToString("0.000000", (IFormatProvider) CultureInfo.InvariantCulture) + "\t" + this.unknown20[index].Y.ToString("0.000000", (IFormatProvider) CultureInfo.InvariantCulture) + "\t" + this.unknown20[index].Z.ToString("0.000000", (IFormatProvider) CultureInfo.InvariantCulture) + "\t" + this.unknown21[index].X.ToString("0.000000", (IFormatProvider) CultureInfo.InvariantCulture) + "\t" + this.unknown21[index].Y.ToString("0.000000", (IFormatProvider) CultureInfo.InvariantCulture) + "\t" + this.unknown21[index].Z.ToString("0.000000", (IFormatProvider) CultureInfo.InvariantCulture));
      outstream.WriteLine("====================================================");
    }

    public override string ToString()
    {
      return "[Geometry] ID: " + (object) this.id + " size: " + (object) this.size + " Count: " + (object) this.vert_count + " Count2: " + (object) this.header_count + " Headers: " + (object) this.headers.Count + " Size: " + (object) this.geometry_size + " Verts: " + (object) this.verts.Count + " UVs: " + (object) this.uvs.Count + " Pattern UVs: " + (object) this.pattern_uvs.Count + " Normals: " + (object) this.normals.Count + " unknown_15: " + (object) this.weight_groups.Count + " unknown_17: " + (object) this.weights.Count + " unknown_20: " + (object) this.unknown20.Count + " unknown_21: " + (object) this.unknown21.Count + " Geometry_unknown_item_data: " + (object) this.unknown_item_data.Count + " unknown_hash: " + StaticStorage.hashindex.GetString(this.hashname) + (this.remaining_data != null ? (object) (" REMAINING DATA! " + (object) this.remaining_data.Length + " bytes") : (object) "");
    }
  }
}
